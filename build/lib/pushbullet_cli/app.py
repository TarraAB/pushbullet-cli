...
def _push(data_type, title=None, message=None, channel=None, device=None, file_path=None, url=None, mime_type=None):
    pb = uploadPb = _get_pb()
    ...
    if data_type == "file":
        with open(file_path, "rb") as f:
            file_data = uploadPb.upload_file(f, os.path.basename(file_path), mime_type)
...
@click.option("-m", "--mime", "--mimetype", default=None, help="The given argument is a MIME type of file to push.")
@click.option("-u", "--link", default=None, help="The given argument URL.")
@click.argument('arg', default=None, required=False)
def push(title, device, channel, filename, mimetype, link, arg):
    if device is not None and channel is not None:
        raise click.ClickException("--channel and --device cannot be used together")
        
    if mimetype is not None and filename is None:
        raise click.ClickException("--mimetype can only be used with filename")
